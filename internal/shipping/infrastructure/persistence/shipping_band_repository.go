...
func NewPostgresShippingBandRepository(db *database.DB) *PostgresShippingBandRepository {
...
func (r *PostgresShippingBandRepository) Delete(ctx context.Context, id int64) error {
	return r.db.Exec(ctx, `DELETE FROM blc_shipping_band WHERE id = $1`, id)
}
...
func (r *PostgresShippingBandRepository) DeleteByMethodID(ctx context.Context, methodID int64) error {
	return r.db.Exec(ctx, `DELETE FROM blc_shipping_band WHERE method_id = $1`, methodID)
}
